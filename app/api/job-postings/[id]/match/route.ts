import { NextRequest } from 'next/server';
import { getToken } from 'next-auth/jwt';
import { ResponseHandler } from '@/utils/response-handler';
import { jobMatchingService } from '@/services/jobs/job-matching.service';
import { ROLES, UserRole } from '@/types/rbac';

const SECRET = process.env.NEXTAUTH_SECRET;

// POST - Manually trigger matching for a job posting
export async function POST(
  req: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    const token = await getToken({ req, secret: SECRET });
    if (!token || !token.sub) {
      return ResponseHandler.unauthorized();
    }

    // Ensure only HR Managers or Super Admins can trigger matching
    if (![ROLES.HR_MANAGER, ROLES.SUPER_ADMIN].includes(token.role as UserRole)) {
      return ResponseHandler.forbidden();
    }

    const jobPostingId = params.id;
    const companyId = token.companyId as string;

    if (!companyId) {
      return ResponseHandler.error('Company ID not found in token');
    }

    // Start matching process in background
    jobMatchingService.manualMatchForJob(jobPostingId, companyId)
      .then(() => {
        console.log(`Manual matching completed for job posting: ${jobPostingId}`);
      })
      .catch((error) => {
        console.error(`Manual matching failed for job posting: ${jobPostingId}:`, error);
      });

    return ResponseHandler.success(
      { message: 'Matching process started' },
      'Matching process has been initiated. This may take a few minutes.',
      202
    );

  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
    return ResponseHandler.error(errorMessage);
  }
}
